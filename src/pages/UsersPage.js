...
export function UsersPage() {
  ...
  const saveUser = async () => {
    ...
    setSubmitting(true);
    try {
      await supabase.from('users').update({ full_name: userForm.full_name, email: userForm.email.toLowerCase().trim(), role: userForm.role, group_id: userForm.group_id || null }).eq('id', editingUser.id);
  ...
  let filteredUsers = users.filter(u => u.full_name.toLowerCase().includes(search.toLowerCase()) || u.email.toLowerCase().includes(search.toLowerCase()));
  if (filter !== 'all') filteredUsers = filteredUsers.filter(u => u.role === filter);

  const filterTabs = [{ id: 'all', label: 'Все' }, ...ROLES.map(r => ({ id: r.id, label: r.label }))];
  ...
  return (
    <>
          ...
          {loading ? (
            ...
            <div className="users-list">
              {filteredUsers.map((user, index) => (
                <div key={user.id} className="user-item" style={{ animationDelay: `${index * 0.03}s` }} onClick={() => openEditModal(user)}>
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="Редактировать пользователя" footer={
        ...
        <FormField label="Имя и фамилия"><Input value={userForm.full_name} onChange={(e) => setUserForm({ ...userForm, full_name: e.target.value })} placeholder="Иванов Иван" autoFocus /></FormField>
        <FormField label="Email"><Input type="email" value={userForm.email} onChange={(e) => setUserForm({ ...userForm, email: e.target.value })} placeholder="email@example.com" /></FormField>
        ...
        <FormField label="Группа">
          <select className="form-select" value={userForm.group_id} onChange={(e) => setUserForm({ ...userForm, group_id: e.target.value })}>
