...
export function SchedulePage() {
  ...
  const [lessonForm, setLessonForm] = useState({ subject: '', teacher: '', room: '', day_of_week: 1, start_time: '08:30', end_time: '10:00', lesson_type: 'lecture' });
  ...
  useEffect(() => { if (selectedGroup) loadSchedule(selectedGroup); }, [selectedGroup]);
  ...
  const openEditModal = (lesson) => {
    ...
    setLessonForm({ subject: lesson.subject, teacher: lesson.teacher || '', room: lesson.room || '', day_of_week: lesson.day_of_week, start_time: lesson.start_time?.slice(0, 5) || '08:30', end_time: lesson.end_time?.slice(0, 5) || '10:00', lesson_type: lesson.lesson_type || 'lecture' });
    setShowModal(true);
    haptic.light();
  };
  ...
  const daySchedule = schedule.filter(s => s.day_of_week === activeDay).sort((a, b) => a.start_time.localeCompare(b.start_time));
    ...
    <>
      ...
      <PullToRefresh onRefresh={handleRefresh}>
        <div className="page-content">
          ...
          <FilterTabs tabs={DAYS.map(d => ({ id: d.id, label: d.short }))} activeTab={activeDay} onChange={setActiveDay} />
          ...
          {loading ? (
            ...
            <div className="schedule-list">
              {daySchedule.map((lesson, index) => (
                <div key={lesson.id} className="schedule-item" style={{ animationDelay: `${index * 0.05}s` }} onClick={() => canEdit && openEditModal(lesson)}>
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={editingLesson ? 'Редактировать' : 'Добавить занятие'} footer={
        ...
        <FormField label="Предмет"><Input value={lessonForm.subject} onChange={(e) => setLessonForm({ ...lessonForm, subject: e.target.value })} placeholder="Математический анализ" autoFocus /></FormField>
        <FormField label="Преподаватель"><Input value={lessonForm.teacher} onChange={(e) => setLessonForm({ ...lessonForm, teacher: e.target.value })} placeholder="Иванов И.И." /></FormField>
        <FormField label="Аудитория"><Input value={lessonForm.room} onChange={(e) => setLessonForm({ ...lessonForm, room: e.target.value })} placeholder="101" /></FormField>
        <FormField label="День недели">
          <select className="form-select" value={lessonForm.day_of_week} onChange={(e) => setLessonForm({ ...lessonForm, day_of_week: parseInt(e.target.value) })}>
            {DAYS.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}
          </select>
        </FormField>
        <div className="form-row">
          <FormField label="Начало">
            <select className="form-select" value={lessonForm.start_time} onChange={(e) => setLessonForm({ ...lessonForm, start_time: e.target.value })}>
              {TIME_SLOTS.map(t => <option key={t} value={t}>{t}</option>)}
            </select>
          </FormField>
          <FormField label="Конец">
            <select className="form-select" value={lessonForm.end_time} onChange={(e) => setLessonForm({ ...lessonForm, end_time: e.target.value })}>
              {TIME_SLOTS.map(t => <option key={t} value={t}>{t}</option>)}
            </select>
          </FormField>
        </div>
        <FormField label="Тип занятия">
          <select className="form-select" value={lessonForm.lesson_type} onChange={(e) => setLessonForm({ ...lessonForm, lesson_type: e.target.value })}>
