...
export function StatCard({ icon, color = 'blue', value, label, delay = 0 }) {
  return (
    <div className="stat-card" style={{ animationDelay: `${delay * 0.05}s` }}>
 ...
 * Page Header
 */
export function PageHeader({ title, action, search, onSearch, onBack }) {
  return (
    <div className="page-header">
      <div className="page-header-left">
        {onBack && (
          <button className="back-button" onClick={() => { onBack(); haptic.light(); }}>
            ‹
          </button>
        )}
 ...
 * Card Components
 */
export function Card({ children, className = '', delay = 0, onClick, ...props }) {
  return (
    <div 
    ...
    <div className="filter-tabs">
      {tabs.map(tab => (
        <button
          ...
          onClick={() => { onChange(tab.id); haptic.light(); }}
        >
          {tab.label}
        </button>
      ))}
