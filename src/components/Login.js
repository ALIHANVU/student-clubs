 ...
 * При регистрации студент выбирает факультет → направление → группу
 */
export function LoginPage({ onLogin }) {
  ...
  // Данные для выбора структуры университета
  ...
  // Загружаем факультеты при открытии формы регистрации
  useEffect(() => {
    if (mode === 'register') {
      loadFaculties();
    }
  ...
  // Загружаем направления при выборе факультета
  useEffect(() => {
    if (selectedFaculty) {
  ...
  // Загружаем группы при выборе направления
  useEffect(() => {
  ...
  const handleRegister = async (e) => {
    ...
    if (!selectedFaculty || !selectedDirection || !selectedGroup) {
      setError('Выберите факультет, направление и группу');
      haptic.error();
      return;
    }
        ...
        {mode === 'login' ? (
          ...
          <form className="login-form" onSubmit={handleRegister}>
            ...
            <FormField label="Направление">
              ...
              >
                <option value="">
                  {selectedFaculty ? 'Выберите направление' : 'Сначала выберите факультет'}
                </option>
                {directions.map(d => (
            ...
            <FormField label="Группа">
              ...
              >
                <option value="">
                  {selectedDirection ? 'Выберите группу' : 'Сначала выберите направление'}
                </option>
                {groups.map(g => (
            ...
            <div className="demo-credentials-list">
              {DEMO_CREDENTIALS.map((cred, index) => (
                <div key={index} className="demo-credential">
                  <span className="demo-label">{cred.label}:</span>
