...
export function LoginPage({ onLogin }) {
  ...
  const loadGroups = async (directionId) => {
    const { data } = await supabase.from('groups').select('id, name, course').eq('direction_id', directionId).order('name');
    setGroups(data || []);
  };
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (fetchError || !data) { setError('Неверный email или пароль'); haptic.error(); setLoading(false); return; }
      localStorage.setItem('uniclub_user', JSON.stringify(data));
      haptic.success(); onLogin(data);
    } catch (err) { setError('Ошибка входа'); haptic.error(); }
  ...
  const handleRegister = async (e) => {
    ...
    } catch (err) { setError('Ошибка регистрации'); haptic.error(); }
    finally { setLoading(false); }
  };
  ...
  return (
    <div className="login-page">
      <div className="login-card">
        ...
        {/* Mode Tabs */}
        <div className="filter-tabs" style={{ marginBottom: 'var(--spacing-lg)' }}>
          <button 
        ...
        {mode === 'login' && (
          <div style={{ 
            ...
            <p style={{ fontSize: 'var(--font-size-footnote)', fontWeight: 600, marginBottom: 'var(--spacing-sm)', color: 'var(--label-secondary)' }}>
              🔐 Тестовые аккаунты
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              {DEMO_CREDENTIALS.map((cred, i) => (
                <div key={i} style={{ fontSize: 'var(--font-size-caption)', color: 'var(--label-tertiary)' }}>
